	...
	uint8_t freq_index)
{
	...
	if ((vcore_index == 0xFF) || (vcore_index == get_vcore_opp)) {
		...
		if (vcore_index < max_vcore_opp) {
			LOG_DBG("vpu bound vcore opp(%d) to %d", vcore_index,
				max_vcore_opp);
	...
	if (freq_index == 0xFF) {
		...
		if (freq_index < max_dsp_freq) {
			LOG_DBG("vpu bound dsp freq(%dMHz) to %dMHz", log_freq,
				log_max_freq);
...
static bool vpu_change_opp(int core, int type)
{
	...
	case OPPTYPE_VCORE:
		...
		LOG_DBG("[vpu_%d] cgopp vcore=%d, ddr=%d\n", core,
	...
	case OPPTYPE_DSPFREQ:
		...
		break;
	default:
		LOG_DBG("unexpected type(%d)", type);
		break;
	}
...
static int vpu_enable_regulator_and_clock(int core)
{
	...
	LOG_DBG("[vpu_%d] adjust(%d,%d) result vcore=%d, ddr=%d\n", core,
...
int vpu_init_hw(int core, struct vpu_device *device)
{
	...
	if (core == 0) {
		...
		LOG_DBG("efuse_data: efuse_data(0x%x)", efuse_data);
		switch (efuse_data) {
		case 0x3: /*b11*/
...
out:
	unlock_command(core);
	vpu_trace_end();
	LOG_DBG("[vpu_%d] hw_boot_sequence -\n", core);
	return ret;
}
...
int vpu_hw_enque_request(int core, struct vpu_request *request)
{
	...
	LOG_DBG("[vpu_%d] start d2d, id/frm (%d/%d)\n", core,
